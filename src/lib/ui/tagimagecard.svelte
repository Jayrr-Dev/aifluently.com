...
{#if data}
	{#each aggregated_data as _, i}
		...
		{#if data}
			<div
				class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-1"
			>
						...
						></div> -->
							...
							<div class="absolute w-full h-full top-[30%] z-10">
								<div id="name-comp" class=" parent-hover overlay-fade">
									<h1
										class="text-3xl sm:text-xl md:text-2xl lg:text-3xl text-center sm:text-center"
									>
							...
							<div class="absolute w-full top-1/2 left-1/2 ml-[-150px] mt-[-40px] h-full">
								<div class="grid grid-cols-1 gap-1">
									<button
										on:click={() => {
											goto(
												`/product/${sluglify(aggregated_data[i].product_table[j].product_name)}`
											);
										}}
