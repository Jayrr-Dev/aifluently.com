...
<div class="card h-[290px] grid grid-cols-1 relative card-hover parent-element">
	...
	<header class="card-header text-center pr-6">
		{#if state.tags[tag] && showTag}
			<div in:fade={{ duration: 800 }}>
				<span class="badge text-4xl p-0 m-0 translate-y-2">
					<Icon icon={getIconForTag(tag)} />
				</span>
			...
			<div in:fade={{ duration: 800 }}>
				<div class="flex justify-start">
					<section class="p-4 w-full">
						<ol>
							{#each state.tagProductDetails[tag] as product, i}
								<!-- ? Tooltip Styles -->
								<div
									...
									{#if windowWidth < 768}
										...
										<a href={product.name.replace(/\s/g, '')}>
											<div class="flex justify-center">
												<button class="btn btn-sm w-[50%] bg-warning-500 border-2 border-black h-6">
													Learn More
												</button>
											</div>
										</a>
	...
	{#if lightswitch}
		...
		></div>
	{:else}
		<div class=" h-16 absolute w-full bottom-10 bg-gradient-to-t from-surface-50 to-white/0"></div>
	{/if}

	<div class="flex justify-center">
		<button class="btn btn-sm w-[85%] h-8 bg-warning-500 border-2 border-black z-[5]">
			More
		</button>
	</div>
</div>
